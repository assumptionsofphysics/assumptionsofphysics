...
\node at (-1.5, 1.25) {$\vec{S} = -\,$curl $\vec{\theta}$};


\end{tikzpicture}
\end{figure}

A way to intuitively understand this is the following. Take a field line $\gamma$, a line that is always tangent to the vector field $\vec{S}$. Between two point, make a small change to it $\gamma'$. Take $\Sigma$ to be a surface enclosed by these lines. The line integral of $-\vec{\theta}$ over its contour is given by the flow of its curl $\vec{S}$ through $\Sigma$. If the variation is small enough, the surface $\Sigma$ is tangent to the vector field $\vec{S}$ since $\gamma$ is always tangent to it. The line integral of $-\vec{\theta}$ over its contour is zero. This means that the line integral of $-\vec{\theta}$ along $\gamma$ is equal to the one along $\gamma'$: small variations of $\gamma$ do not change the line integral.
