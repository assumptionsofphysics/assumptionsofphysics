...
\begin{defn}[Common sets]
	 The \textbf{empty set}
	 \marginleft{Common sets: $\emptyset$, $\mathbb{N}$, $\mathbb{Z}$, $\mathbb{Q}$, $\mathbb{R}$, $\mathbb{C}$}
...
\begin{defn}[Set operations]
	...
	Let $X \subseteq \mathcal{P}(A)$ be a set of subsets of $A$. The operations of union $\bigcup\limits_{Y \in X} Y$ and intersection $\bigcap\limits_{Y \in X} Y$ can be extend to span all elements.
\end{defn}
...
\begin{defn}[Partitions]
	...
	\begin{enumerate}
		...
		\item $X \cap Y = \emptyset$ for all $X, Y \in P$ where $X \neq Y$
	\end{enumerate}
\end{defn}
