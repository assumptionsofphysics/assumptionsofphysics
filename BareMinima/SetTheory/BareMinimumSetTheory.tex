...
\section{Transfinite (i.e. infinite) numbers}
\begin{remark}
	Cardinal numbers (i.e. one, two, three, ...) identify the number of elements in a set. Ordinal numbers (i.e. first, second, third) identify a position within a sequence. Cardinals and ordinals are in one to one correspondence only for finite values.
\end{remark}

\subsection{Ordinal numbers}

\begin{remark}	
	To define ordinals, we extend the notion of sequences to arbitrary levels of infinity using well-ordered sets.
\end{remark}

\begin{defn}[POS]
	A \textbf{partially ordered set}, \marginleft{POS: $(A, \leq_A)$} or \textbf{POS}, is a set with an associated partial order. That is, a tuple $(A, \leq_A)$ where $A$ is a set and $\leq_A$ is a partial order over $A$. A \textbf{linearly ordered set} is a POS whose order is linear.
\end{defn}

\begin{defn}[Initial segment]
	The \textbf{initial segment} \marginleft{Initial segment} of a POS $(A, \leq_A)$ determined by $a \in A$ is the set of all elements before $a$. That is, $S_a=\{ x \in A \, | \, x < a \}$.
\end{defn}

\begin{defn}[Order morphisms]
	Let $(A, \leq_A)$ \marginleft{Order homo/ \\ isomorphism} and $(B, \leq_B)$ be two POSs. An \textbf{order homomorphism} is a function $f : A \to B$ that preserves the ordering. That is, $a_1 \leq_A a_2$ implies $f(a_1) \leq_B f(a_2)$. An \textbf{order isomorphism} is a bijective order homomorphism whose inverse is also an order homomorphism (i.e. the POSs have identical order). That is, $b_1 \leq_B b_2$ implies $f^{-1}(b_1) \leq_A f^{-1}(b_2)$.
\end{defn}
...
\begin{defn}[Ordinal multiplication]
	The multiplication \marginleft{Ordinal multiplication: $\langle A \rangle \langle B \rangle$} of two ordinals is defined by ordering their Cartesian product. Let $(A, \leq_A)$  and $(B, \leq_B)$ be two well ordered sets. Let $(A \times B, \leq)$ be the POS where:
	\begin{itemize}
...
\begin{prop}[Cantor's theorem]
	A set has strictly lower cardinality than its power set. That is, $|A| < |\mathcal{P}(A)|$. If $a = |A|$ then we note $|\mathcal{P}(A)| = 2^a$
\end{prop}

\begin{prop}[Aleph numbers]
	The cardinality \marginleft{Aleph \\numbers: $\aleph_a$} of infinite sets are well-ordered. They are noted $\aleph_a$ where $a \in \ordinals$.
\end{prop}
...
\begin{remark}
	The continuum hypothesis is proven to be independent on the axioms of set theory and is therefore an additional optional axiom. As the axiom deals with sets of cardinality lower than the continuum, this aspect of set theory may be interesting for those working on the Assumptions of Physics. It could be, in principle, that the distinguishable configurations in space-time have a cardinality that is neither countable nor that of the continuum.
\end{remark}
...
\textbf{Axioms of set theory}. Here are the axioms of a particular formulation of axiomatic set theory taken from CITE:
\begin{enumerate}
...
\begin{remark}
	...
	A lot of work in set theory has been and is to clarify the relationship between the axioms, in particular which are independent from the others. The axiom of choice has been controversial for some time and therefore has been one of the most studied.
\end{remark}
...
\begin{prop}
	...
	\begin{itemize}
		...
		\item Zorn's lemma: every non-empty POS for which every chain (i.e. totally ordered subset) has an upper bound contains at least one maximal element
		\item Every vector space has a basis
	\end{itemize}
\end{prop}

\begin{remark}
	It should be stressed that there are different approaches. Constructive set theory, as all constructive mathematics, rejects ``nonconstructive proofs'' in which the existence of an object is proved with no explicit algorithm as to how to constructive. This approach will, for example, exclude the axiom of choice. For those working on the Assumptions of Physics, then, it is important to understand that the foundations of math are not absolute. Therefore we have to understand the consequences of those choice and understand whether they are suitable for the foundations of physics.
\end{remark}
