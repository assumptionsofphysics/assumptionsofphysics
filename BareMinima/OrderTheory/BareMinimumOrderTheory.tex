...
\begin{defn}[Subposet]
	Given a poset $(X, \leq)$, a \textbf{subposet} is a poset $(Y, \leq_Y)$ where $Y \subset X$ and $\leq_Y = \leq |_Y$.
\end{defn}
...
\begin{defn}[Discrete order]
	A \textbf{discrete order} \marginleft{Discrete order} is an order in which every element that has a predecessor/successor has an immediate predecessor/successor.
\end{defn}

Note that discrete order may also mean an order in which every element is only related to itself (i.e. the partial order is an equivalence relation).
...
\begin{defn}[Lattice - order theoretic]
	A \textbf{lattice} \marginleft{Lattice} is a partially ordered set in which every pair of elements has both a supremum/join and an infimum/meet. A \textbf{sublattice} is a subposet of a lattice.
\end{defn}
...
\begin{prop}
	A lattice is distributive if an only if it does not contain an $M_3$ (diamond) or $N_5$ (pentagon) sublattice.
\end{prop}
