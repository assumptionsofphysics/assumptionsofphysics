...
\begin{remark}
	To give a sense of the broad range of applicability: sets are ordered by set inclusion ($\{0, 1\} \subseteq \{0, 1, 2\}$); statements are ordered by implication/narrowness/specificity (\statement{Snoopy is a dog} $\narrower$ \statement{Snoopy is a mammal}); systems are ordered by is-a-part-of (`piston' $\subseteq$ `engine' $\subseteq$ `car').
\end{remark}
...
\begin{defn}
	An \textbf{increasing function} \marginleft{Increasing function} (also called \textbf{order preserving}) is a map between two posets $(X, \leq_X)$ and $(Y, \leq_Y)$ that preserves the order. That is, a map $f : X \to Y$ such that $a \leq_X b \implies f(a) \leq_Y f(b)$. It is \textbf{strictly increasing} if $a \leq_X b \implies f(a) <_Y f(b)$.
\end{defn}

\begin{defn}
	A \textbf{decreasing function} \marginleft{Decreasing function} (also called \textbf{order reversing}) is a map between two posets $(X, \leq_X)$ and $(Y, \leq_Y)$ that reverses the order. That is, a map $f : X \to Y$ such that $a \leq_X b \implies f(b) \leq_Y f(a)$.  It is \textbf{strictly decreasing} if $a \leq_X b \implies f(b) <_Y f(a)$.
\end{defn}
...
\begin{defn}
	An \textbf{order embedding} \marginleft{Order embedding} is a monotonic function where the image have the same ordering. That is, a map $f : X \to Y$ such that $a \leq_X b \iff f(a) \leq_Y f(b)$.
\end{defn}

\begin{defn}
	An \textbf{order isomorphism} \marginleft{Order isomorphism} is an invertible embedding (i.e. bijective monotonic function whose inverse is monotonic). Two posets are \textbf{order isomorphic} (or have the same \textbf{order type}) if there exists an order isomorphism between them.
\end{defn}

\subsection{Filters and ideals AFTER}
...
\begin{remark}
	The upper/lowers bound of $A$ are not necessarily in $A$.
\end{remark}
...
\begin{defn}[Lattice properties]
	...
	\begin{description}
		...
		\item[Complemented.] A bounded lattice is \textbf{complemented} if every element has a complement. It is uniquely complement if the complement is unique.
		\item[Ortho-complemented.] A bounded lattice is \textbf{complemented} if every element has a unique complement.
	\end{description}
\end{defn}
