...
\begin{defn}[Monotone continuity]
...
We therefore want to make the following distinction. TODO: define countable vs uncountable

\begin{defn}
	We say that $\stmt$ is a \textbf{uncountable} infinitesimal with respect to $\stmt[u]$ if all there exists a sequences $\{ \stmt_i \}_{i=1}^\infty$ such that:
	\begin{enumerate}
		\item $\stmt \finer \stmt_i$ for all $i$
		\item $\stmt_i \ncomp \stmt_j$ for all $i \neq j$
		\item $\bigOR\limits_{i=1}^n \stmt_i \finer \stmt[u]$ 
	\end{enumerate}
	In words, $\stmt$ is a countable infinitesimal with respect to $\stmt[u]$ if we can find for any number, there is a disjunction of that many broader mutually incompatible statements $\stmt_i$ is finer than $\stmt[u]$.
\end{defn}


TODO: desiderata that if there are no countable infinitesimal, then we have monotone continuity

We wish to understand the following. This is monotone continuity up to infinitesimals, and so with the right definitions, it should translate to monotone continuity for $\lequ$. 

Let $\stmt[u]$ be a statement which does not admit countable infinitesimals, and let $\{\stmt_i:i\in\mathbb{N}\}$ be a sequence of statements monotone increasing in narrowness (i.e. $\stmt_i\narrower\stmt_j$ for all $i\leq j$). Write $\stmt=\OR_i\stmt_i$ for the disjunction and assume $\stmt$ is finitely comparable to $\stmt[u]$. If $\stmt[t]\in\tdomain$ such that $\stmt[t]$ is finitely comparable to $\stmt[u]$, and $\stmt_i\finer\stmt[t]$ for all $i$, then $\stmt\finer\stmt[t]\OR\stmt[w]$ for some infinitesimal (with respect to $\stmt[u]$) $\stmt[w]$. 
...
TODO: Make this an example of how measure zero is not equigranularity
...
The issue is that the granularity of a countable sequence with each term a finite size but an infinite limit ``jumps'' at infinity. For example, take the sequence $\{1\}$, $\{1, 3\}$, $\{1, 3, 5\}$, ... , \{ all odd natural numbers\}. Compare to $\{1\}$, $\{1, 2\}$, $\{1, 2, 3\}$, ... , \{ all natural numbers\}. They are both maximal chains. They both go through all sizes of finite sets, but they reach infinity at two different levels. Effectively, the above property is only true over finite sets.


The following consequences of the MCP are interesting, but only use much weaker statements about the fineness classes of possibilities. 
...
TODO: add remark that MCP -> finite domain
