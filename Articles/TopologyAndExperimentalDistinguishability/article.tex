...
\begin{proof}
	The collection $\mathsf{T}(C(X,Y), \mathcal{B}_X, \mathcal{B}_Y)$ is defined to be the topology generated by the sets $V(U_X,U_Y)$, so it contains the empty set, all of $C(X,Y)$, and is closed under arbitrary union and finite intersection by definition.
\end{proof}
