...
Though we believe the arguments to be thoroughly convincing, we are stopping short from declaring Schwartz spaces physical. First, because this is not the goal of the paper, which is just to show the unphysicality of Hilbert spaces. Second, because we would need to show the importance of expectation values of polynomials from experimental considerations. It is not clear to us that that's the case, so it may be that the justification would have to proceed in a different, but ultimately equivalent, way.
