...
To summarize, we are not interested in understanding whether Hilbert spaces have nice mathematical features for calculating or proving results. The fact that other tools have been developed to solve other mathematical problems is not relevant to this discussion. We are only interested in the question of mathematical representation of quantum states, which are physical entities we prepare in a lab. Something we produce, manipulate and, in the end, measure. What physical requirements are we capturing when we model them mathematically? Are Hilbert spaces capturing, and only capturing, physically tenable assumptions, or not?
