...
\title{A non-additive generalization of probability and \\ state count for all physical theories}
...
\begin{abstract}
	We present a physically motivated non-additive generalization of probability and state counting measures that is equally valid on all physical theories that allow a statistical description. In the classical case, these reduce to the standard probability and Liouville/counting measures, but retain useful properties in the general case. Both constructions are instances of a more general construction that may may be of more general interest. Moreover, the expectation for statistical variables is not recovered by standard non-additive integrals, and therefore new techniques will need to be developed.
...
In the past few decades, there has been a growing interest in developing approaches that generalize the notion of states and physical theories in terms of probabilistic and/or entropic concepts \cite{plavala2023,wilce2025,ariano2017,caticha2019,hall2016}, which stems from the underlying aim to characterize physical theories from an operational viewpoint. A key insight is that statistical/probabilistic concepts often provide a suitable basis for this generalization, and it is in this setting where most of these approaches operate.

A key problem, then, is what notion of probability should be used given that the standard measure theoretic Kolmogorovian approach does not generally work in quantum mechanics\cite{chsh1969,fine1982}. The default approach is simply to view quantum states as collections of probability measures over different quantities. Another approach is to extend probability measures to signed measures\cite{zachos2005,mandel1995,monchietti2025}, though it is not clear what negative probability represent, particularly since non-negative quasiprobability representations of quantum spaces exist. Yet another approach is to replace the $\sigma$-algebra with a different structure (CITE some quantum logic, quantum measure theory ). The situation, then, is that we multiple approaches, each with its own strength and weakness, with no clear connection or straight forward physical justification. Moreover, none of them generalize the connection between entropy and the classical Liouville measure for counting states.

The aim of this paper is to present a simple, physically motivated structure in which all these approaches can be connected. Additionally, we show that a natural extension of probability and state counting measures exist under very general conditions.

The main findings are the following:
\begin{itemize}
	\item any physical theory needs to provide a space of statistical ensemble, which need to be a bounded convex subsets of a locally convex second countable topological vector space
	\item Choquet theory represents each ensemble $\ens$ with one or more probability measures $p_{\ens}$ on the set of extreme points $X$ (i.e. pure states)
	\item if this representation is unique (i.e. the ensemble space is a simplex) classical probability theory can be used
	\item the classical case corresponds to pure states being mutually exclusive
	\item quasiprobability approaches provide affine representions of ensembles in terms of a function space, whose points are unrelated to the extreme points
	\item the non-positivity of the measures is not key to quasiprobability approaches, and classical spaces admit quasiprobability as well
	\item the algebras used in quantum logic and quantum measure theory can always be understood as sub-algebras of the Borel algebra of ensembles
	\item a generalization of probability measures $p_{\ens}(A)$ emerges by asking the question ``what is the biggest fraction of a particular ensemble $\ens$ that can be expressed as a mixture of ensembles of $A$?''
	\item on the set of extreme points $X$, the generalized probability measure is the supremum of all Choquet representations, and becomes additive in the classical case
	\item a generalization of the state counting measure $\mu(A)$ emerges by asking ``what is the biggest spread that can be achieved with a mixture of ensembles of $A$?''
	\item  the generalized state counting measure reduces to the Liouville measure in the classical case, and to the dimensionality of subspaces in the quantum case
	\item to properly generalize probability theory, a non-additive notion of integral and derivative is needed which does not seem to match standard approaches.
\end{itemize}

A final remark before we start. Too often works in the foundations of physics focus on a particular aspect without concern to a broader connection to all relevant mathematical, physical and philosophical aspects, such as information geometry, functional analysis, quantum logic and so on. At this point in time, what is needed is a synthesis of many different idea. However, such work carried to completion would be effectively overwhelming both to produce and to read. Therefore, the main part of the paper can only highlight the main conceptual results regarding measure theoretic representations. In the appendix wit go through some of the relevant mathematical detail while all the rest is left to our evolving collaborative open source work.\cite{aop-book} 
...
The above two approaches exhaust what can be done with additive measures, so we turn to the non-additive case. We found a general construction that, given a function $f: \Ens \to \mathbb{R}$, provides us with a set function $cs_f(A)$ that returns the supremum of $f$ over the convex hull of $A$. This convex supremum is always a monotone set function continuous from below. The generalization of both probability measures and state counting measures are applications of this construction.

To generalize the notion of probability measure, given a target ensemble $\ens$, we can define the fraction $\fraction_{\ens}(\ens[a])$ to be the highest possible $p$ such that $\ens = p \ens[a] + (1-p) \ens[b]$. The fraction capacity $\frcap_{\ens}(A)$ is the corresponding convex supremum, which returns what fraction of $\ens$ can be produced by a mixture of a given set of ensembles $A$. The fraction capacity is found to be a monotone, non-negative, bounded by one, continuous from below, sub-additive set function. Over the extreme points, the fraction capacity represents the supremum of all possible measures that can represent the same ensemble in Choquet theory. The classical case exactly corresponds to the simplex case, in which fraction capacity corresponds to a unique measure, and is therefore additive. This again solidifies the connection between classical probability, additivity of measures and mutual exclusivity.

To generalize the notion of count of states, we note that in classical statistical mechanics the entropy over uniform distribution is the logarithm of the count of states in the region, which is the supremum of the entropy of all measures with the same support. We proceed in the opposite way: for each ensemble we can define the exponential of the entropy (i.e.~its entropic volume)\cite{hall1999} and define the state capacity to be the corresponding convex supremum. The state capacity is found to be monotone, non-negative, continuous from below, sub-additive set function. It reduces to the Liouville measure in the classical case and to the dimensionality of subspaces in the quantum case.
...
\textbf{Basic requirements for an ensemble space}. In our larger work, we strive to give all axioms and mathematical definitions solid grounding in physical requirements.\footnote{We call this approach Physical Mathematics, which stems from the observation that when physical objects are mapped to the right mathematical objects, the physical specification maps to the mathematical definitions.} Here we will only present a summary for the present investigation, leaving details to the appendix and our ongoing work.\cite{aop-book}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[x={(\xx cm, \xy cm)}, y={(\yx cm, \yy cm)}, z={(\zx cm, \zy cm)}, scale=1.2]
		...
		\end{scope}
	\end{tikzpicture}
	\caption{Examples of a classical discrete ensemble space with three pure states (triangle) and of a two dimensional quantum ensemble space (ball). Infinite dimensional spaces, like continuous classical ensemble spaces and quantum ensemble space with continuous quantities cannot be visually represented. }\label{convex_examples}
\end{figure}

Since physical laws deal with experimentally reproducible relationships, they are inherently relationships between statistical ensembles. Therefore, any physical theory must define the space of all ensembles allowed within the theory. In the classical case, this corresponds to a probability distribution over a discrete set of outcomes (e.g. a fair coin) or over phase space (e.g. the symplectic manifold representing position and momentum for a classical particle). In the quantum case, this corresponds to a mixed state (e.g. a positive semi-definite self-adjoint operator of trace one). See Fig.~\ref{convex_examples} for examples. 

In this work, we will take an ensemble space $\Ens$ to be a bounded convex subset of a locally convex metrizable second countable topological vector space endowed with an entropy function. The second countable topology captures experimental verifiability as open sets represent experimentally verifiable statements\cite{aop-book,kelly1996}.\footnote{A verifiable statement is an assertion for which there exists an experimental test that finishes successfully in finite time if and only if the statement is true.} Convexity, and linearity more in general, comes from allowing statistical mixtures: if $\ens[a],\ens[b] \in \Ens$ are ensembles, then $p \ens[a] + \bar{p} \ens[b]$ represents the statistical mixture where $\ens[a]$ is taken with probability $p$ and $\ens[b]$ with probability $\bar{p} = 1 - p$.\footnote{Only finite mixtures are guaranteed, while the topology will decide whether an infinite mixture $\sum_{i=1}^{\infty} p_i \ens_i$ converges in the space or not.} Lastly, the entropy characterizes the variability of the elements within an ensemble. The entropy is a function $S : \Ens \to \mathbb{R}$ that satisfies the following bounds
\begin{equation}
	\begin{aligned}
...
To summarize, a repeatedly verifiable physical theory requires ensembles, which must be defined experimentally, allow statistical mixing and provide an entropy. Additionally, we require the existence of enough statistical quantities to identify each ensemble. We therefore take an ensemble space $\Ens$ to be a bounded convex subset of a metrizable second countable locally convex topological vector space $V$. In some cases, for simplicity, we will require $\Ens$ to admit a ``compact closure'' $E$, so that it includes the set $X$ of its extreme points. We now turn to the question of how we can represent both the entropy and each ensemble in terms of some measure theoretic structure.
...
\section{Convex supremum}

The set functions we are going to construct both can be understood as coming from the same construction that has very general applicability. Let $\Ens$ be a convex space and $f : \Ens \to \mathbb{R}$ a real-valued function defined on it. Recall that, given a subset $A \subseteq \Ens$, its convex hull $\hull(A)$ includes all possible convex combinations of elements of $A$. Given a set $A$, then, we can ask what is the highest value that $f$ can reach over $\hull(A)$. If $A$ is the empty set, this would not be defined, so we need a value $f_{\emptyset}$ for the empty set such that $f_{\emptyset} \leq \inf(f(\Ens))$ to preserve monotonicity. Given $f$ and $f_{\emptyset}$, we can define the convex supreme as $cs_f(A) = \sup(f(\hull(A)) \cup \{f_{\emptyset}\})$. This will be a monotone set function continuous from below and with a range of $[f_{\emptyset}, \sup(f(\Ens))]$.

To put these properties in context, a monotone set function continuous from below is a measure if and only if it is additionally non-negative and finitely additive. Therefore, this construction naturally gives well behaved non-additive measures and, since our measures come from this construction, it may be useful to study this class of set functions more in general. 
...
\end{equation}
This returns the biggest fraction of $\ens$ that can be achieved with a mixture of elements of $A$. This is exactly the convex supremum of the fraction with $f_{\emptyset} = 0$.
...
\end{equation}
This returns the upper bound to the exponential of entropy achievable with a mixture of elements of $A$.  This is exactly the convex supremum of the exponential of the entropy with $f_{\emptyset} = 0$.
...
\section{Quantum measures and quantum logic}

Note that our approach also connects, in a sense, to what is done in quantum measure theory and quantum logic.(TODO cite) In these approaches, the probability measure is not defined on Borel set, but rather on the lattice of closed subspaces. While we work on the ensemble space rather than space of pure states, the notion of subspace in two spaces is equivalent. If we restrict the state capacity on the lattice of orthogonal subspaces, then, one has $\capacity(X \vee Y) = \capacity(X) + \capacity (Y)$ if $X \perp Y$. That is, the measure is additive over orthogonal sets, instead of disjoint, sets. Similarly, given an observable $O$, one can define the set $\mathcal{C}$ of all subspaces that correspond to the Borel sets of $O$. In the ensemble space, these correspond to the closed convex subspaces of all possible mixtures of elements whose wavefunction have support within the corresponding Borel set. All $X,Y \in \mathcal{C}$ will either orthogonal (i.e. $X \perp Y$) or one a subspace of the other (i.e. $X \subseteq Y$ or $Y \subseteq X$). If a density operator commutes of $O$, then we can always express it as a mixture of elements of $X$ and $X^{\perp}$ for all $X \in \mathcal{C}$. In this case, $\frcap_{\ens}$ is additive if restricted on $\mathcal{C}$, which means we recover the probability measures associated with the observable.

It is likely that there are further connection that could be made, but since quantum measures and quantum logic are not, strictly speaking, measure theoretic tools, we will leave this for future work.
