...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=3]
		% axes
		\draw[->] (0, 0) -- (1.3, 0);
		\draw (0, 0) node[left] {$0$} -- +(-\ticklength, 0) (0, 0.5) node[left] {$1/2$} -- +(-\ticklength, 0) (0, 1) node[left] {$1$} -- +(-\ticklength, 0);
		\draw[->] (0, 0) -- (0, 1.1);
		\draw (0, 0) node[below] {$a$} -- +(0, -\ticklength) (0.5, 0) node[below] {$1/2(a+b)$} -- +(0, -\ticklength) (1, 0) node[below] {$b$} -- +(0, -\ticklength);
		% cases s <= 1/2(a+b) and 1/2(a+b) < s <= b
		\draw[thick, blue, domain=0.5:1, samples=200] (0, 1) --plot (\x, {0.5/\x});
		% mark discontinuity
		\filldraw (1, 0.5) circle (0.02cm);
		\draw (1, 0) circle (0.02cm);
		% case s > b
