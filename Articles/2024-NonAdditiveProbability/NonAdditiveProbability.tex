...
In this work, we will take an ensemble space $\Ens$ to be a bounded convex subset of a locally convex metrizable second countable topological vector space endowed with an entropy function. The topology captures experimental verifiability as open sets represent experimentally verifiable statements\cite{aop-book,kelly1996}.\footnote{A verifiable statement is an assertion for which there exists an experimental test that finishes successfully in finite time if and only if the statement is true.} Convexity, and linearity more in general, comes from allowing statistical mixtures: if $\ens[a],\ens[b] \in \Ens$ are ensembles, then $p \ens[a] + \bar{p} \ens[b]$ represents the statistical mixture where $\ens[a]$ is taken with probability $p$ and $\ens[b]$ with probability $\bar{p} = 1 - p$.\footnote{Only finite mixtures are guaranteed, while the topology will decide whether an infinite mixture $\sum_{i=1}^{\infty} p_i \ens_i$ converges in the space or not.} Lastly, the entropy characterizes the variability of the elements within an ensemble. In this context, a statistical quantity, representing the expectation of a real valued quantity, is an affine continuous function $F : \Ens \to \mathbb{R}$. That is, $F(p \ens[a] + \bar{p} \ens[b]) = p F(\ens[a]) + \bar{p} F(\ens[b])$.
