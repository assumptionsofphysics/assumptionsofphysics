...
While the discussion has been focused mainly on whether $\psi$-epistemic models are problematic or whether the PBR argument itself is problematic, we ask a different question: what if the underlying HS classification itself is problematic? In particular, the framework assumes that once pure states are fully characterized, mixed states are constructed simply as a standard (i.e. classical) statistical mixture. However, as is well-known, classical and quantum mixtures have different properties. For instance, classical mixtures have a single decomposition in terms of pure states while quantum mixtures do not. Studying the behavior of states under mixing, then, provides a new way to explore \textcolor{blue}{possible violations}. %of what? I would also mention that the example will be relevant for our argument.

\textcolor{blue}{Using this new approach}, %which new approach? Io utilizzerei la frase che era presente nella versione precedente: ``using information theoretic considerations we show that $\psi$-ontic models cannot reproduce all the predictions, results and implications of QM since the lack of overlap of epistemic states---a necessary condition for a model to be $\psi$-ontic---requires that all states must be orthogonal, which openly contradicts quantum theory.''
in this paper we are going to provide a no-go theorem for $\psi$-ontic models. \textcolor{red}{The core insight is that the degree of overlap between two probability distributions affects how the entropy of their mixture relates to the entropy of the components.} %contrary to some previous version of the paper, this sentence is not put in context and some readers not familiar with HS will not understand. 
Consequently, we find that the lack of overlap required by $\psi$-ontic models forces all pure states to be orthogonal. Since this directly contradicts quantum theory, we conclude that $\psi$-ontic models are not viable.
...
%The exposition of the: ci possono obiettare che  la nostra esposizione a non essere fedele, quando invece  proprio il framework di HS d avere il problema di cui parliamo.
Looking at the HS classification it seems that one has complete freedom in choosing the overlap between epistemic states. %for argumentative strategies I would explain why it seems so.
In this section we will show that this is not the case: this choice is in fact constrained by entropy of mixtures. Moreover, these constraints cannot be replicated by a $\psi$-ontic model with standard (i.e.\ classical) information theoretic techniques. Since quantum statistical mechanics and quantum information theory depend on the correct value of entropy on mixtures, $\psi$-ontic models will violate quantum theory. %to make our argument stronger we should show that psi-ontic models rely on classical information theoretic structures. This fact should clearly stated now not in the next paragraph.

To show the violation, we will first study how the entropy behaves when mixing two states, both for the Shannon/Gibbs entropy and the von Neumann entropy.\footnote{Full calculations with all steps are included in the appendix.} The first will be used to calculate entropy of statistical mixtures of epistemic states, since equation \ref{epistemic_mixing} tells us the mixture is done according to a standard (classical) measure of probability. The second will be used to calculate the entropy on the quantum case. % always for the sake of clarity, here I would anticipate what we are going to say: given the formal structure on which psi-ontic models are built they cannot reproduce QM. In this way the argument will be already "in the head" of our readers.
...
It is well established that classical probability theory cannot recover the probability of quantum transitions. So, how does the HS model try to accommodate this? Given that $p(\lambda|P)$ combines using classical probability theory, all the non-classicality must be in $p(k|\lambda, M)$. In fact, note that this second expression does not allow us to write the joint probability distribution for multiple observers; therefore, each ontic state is not partitioned further. Therefore if $\lambda$ is the state of a single quantum system, we can hide the inner product inside $p(k|\lambda, M)$. This is enough to hide our non-classical probability for pure states of single quantum systems, but not much more.

In our result, we saw that the use of standard measure theory when creating mixtures creates a problem because the convex space of classical distributions is significantly different from the convex space of quantum mixed states. For this case, ontological models are ``not quantum enough.''
...
\end{equation}
there is a problem. The issue here is not the factorization. The issue here is breaking the composite system into parts, each with its own ontic state as in 
\begin{equation}
	\begin{aligned}
...
Therefore there are two different ways ontological models use classical measure theory: one in mixtures, highlighted by us; one in composite systems, highlighted by PBR. The use of classical measure theory is what drives the failure of the model.
...
Our conclusion is that the PBR theorem finds that $\psi$-epistemic models are not sustainable because of an underlying issue of the HS definitions, not because epistemic interpretations contradict quantum theory. Specifically, the problem of HS approach is the use of standard probability theory in composite systems. If understood in this manner, the PBR result is the first part of a more general argument showing the inadequacy of the HS framework to categorize quantum theories, an argument that we have completed with the proposed no-go theorem against $\psi$-ontic models.
