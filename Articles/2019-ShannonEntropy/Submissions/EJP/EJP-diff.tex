\documentclass[iopart]{revtex4-1} 
%DIF LATEXDIFF DIFFERENCE FILE


% The line above defines the type of LaTeX document.
% Note that EJP is an IOP journal and should use iopart.cls, but that iopart.cls is supposedly incompatible with amsmath.sty.
...
The \DIFdelbegin \DIFdel{Shannon entropy }\DIFdelend \DIFaddbegin \DIFadd{goal of this work is to give a comprehensive introduction to the concept of Shannon entropy, the expression }\DIFaddend $-\sum p_i \log p_i$\DIFaddbegin \DIFadd{, which }\DIFaddend is well established in such disparate fields as computer science, \DIFdelbegin \DIFdel{physics\mbox{%DIFAUXCMD
\cite{Jaynes1957-1, Grandy} }\hspace{0pt}%DIFAUXCMD
and ecology , yet it }\DIFdelend \DIFaddbegin \DIFadd{communication theory, physics \mbox{%DIFAUXCMD
\cite{Jaynes1957-1, Grandy} }\hspace{0pt}%DIFAUXCMD
ecology and economics. The concept of entropy in physics, despite being treated extensively in the physics education literature (e.g. \mbox{%DIFAUXCMD
\cite{baierlein1994entropy, carson2002undergraduate, Santillan_2008, LeffPartI, LeffPartII, LeffPartIII, LeffPartIV, LeffPartV, Benguigui_2013, geller2014entropy, Tiwary_2020}}\hspace{0pt}%DIFAUXCMD
), }\DIFaddend is still surrounded by a great deal of confusion \DIFdelbegin \DIFdel{.\mbox{%DIFAUXCMD
\cite{Swendsen, KishFerry} }\hspace{0pt}%DIFAUXCMD
For example, in statistical mechanics it is often introduced and associated with vague notions (such as uncertainty, knowledge, lack of knowledge and disorder) which prevent a proper understanding of the concept.  \mbox{%DIFAUXCMD
\cite{Styer, Ben-Naim} }\hspace{0pt}%DIFAUXCMD
In this work we concentrate on giving a better characterization of the Shannon entropy and }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Swendsen, Styer}}\hspace{0pt}%DIFAUXCMD
. Partly because, at a fundamental level, there is still disagreement on what is the ``correct'' notion of entropy, particularly out of equilibrium \mbox{%DIFAUXCMD
\cite{jarzynski2017stochastic, goldstein2019gibbs, maes2003time}}\hspace{0pt}%DIFAUXCMD
. Partly because the same word ``entropy'' is used to refer to concepts (e.g. topological entropy, thermodynamic entropy, graph entropy, R\'{e}nyi entropy, information entropy) that have different definitions and various degrees of overlap.  We will focus on the notion of Shannon entropy, by which we mean the use of $- \sum_i p_i \log p_i$ in the broadest characterization, which has received less attention in physics education. We will see that the Shannon entropy can be given a crisp and precise characterization and will explore }\DIFaddend its relation to \DIFdelbegin \DIFdel{statistical mechanics by addressing what we believe are the }\DIFdelend \DIFaddbegin \DIFadd{the Gibbs and Boltzmann entropies in statistical mechanics.}\footnote{\DIFadd{We will not discuss its relation to thermodynamic entropy. Thermodynamic entropy is defined on the notions of heat and work while the Shannon entropy is defined on the notion of a distribution. The contexts are different and relating them goes beyond the scope of this work. }} \DIFadd{The same material is also presented in a series of videos \mbox{%DIFAUXCMD
\cite{videos}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{To give a crisp and intuitive meaning to the Shannon entropy we need to address }\DIFaddend two main problems\DIFdelbegin \DIFdel{; we do not discuss the relationship to the thermodynamic entropy, which is separate concept. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend The first problem is \DIFdelbegin \DIFdel{that the formula is attached to different }\DIFdelend \DIFaddbegin \DIFadd{a consequence of its success. As it is used in many fields, it is generally introduced with }\DIFaddend concepts that are specific to \DIFdelbegin \DIFdel{each field . To disentangle this issue we need }\DIFdelend \DIFaddbegin \DIFadd{that field but may not be appropriate for others. As we will see, the meaning of the $p_i$ can vary significantly. The first goal, then, is }\DIFaddend to provide a characterization of the Shannon entropy that satisfies the following requirements:
\begin{itemize}
...
\end{enumerate}
These are the same requirements Shannon put forth for his expression\cite{Shannon}, from which he showed that the only possible choice is $H(p_i) = - \sum p_i \log(p_i)$. \DIFaddbegin \DIFadd{See also \mbox{%DIFAUXCMD
\cite{khinchin2013mathematical} }\hspace{0pt}%DIFAUXCMD
for a similar derivation. }\DIFaddend This expression quantifies the variability of the elements within the distribution, the variety of values one finds. The precise meaning of this variability is context dependent, as the choices of the elements and binning are not fixed and the meaning of the weights depends on what the distribution is describing. But this is true for any mathematical object: a real number may represent mass, color in the frequency spectrum, the total money supply, the half-life for an isotope, a probability and so on.
...
\noindent
For the first question, we group cats on one side and everything else on the other, to form a 48/52 split. So we can ask, ``Is it a cat?". If the answer is yes, we finished. If not, we need to continue. We can group dogs on one side and everything else on the other to form a 27/25 split. So we can ask, ``Is it a dog?". If the answer is yes, we are done. If not, we continue. We can group fish with reptiles and birds with small mammals to form a 13/\DIFdelbegin \DIFdel{14 }\DIFdelend \DIFaddbegin \DIFadd{12 }\DIFaddend split. So we can ask, ``Is it a fish or a reptile?". If the answer is yes, the followup question would be ``Is it a fish?". If the answer is no, the followup question would be ``Is it a bird?". With this scheme, the encoding, where 1 represents 'yes' and 0 represents 'no' to the each of the questions asked, is as follows:
\begin{itemize}
...
When considering continuous distributions, we have to remember that the densities are given per unit. That is, $\rho(x)$ has dimensions $[\frac{1}{x}]$. The variability will be also given relative to the unit. For example, a uniform distribution over a unit interval will correspond to zero variability. A uniform distribution over a two unit interval will correspond to one bit, since we will need one bit to narrow the variability back to one unit. A distribution over half a unit interval will correspond to minus one bit, since we would need to ``lose'' one bit of information to widen the variability back to one unit. In other words, the variability is measured by the number of bits needed to identify an element up to one unit\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, up to the resolution (i.e. the granularity) given by that unit.
}\DIFaddend 

This means that infinitesimally narrow distributions, like delta functions, do not work well with the Shannon variability as they would give minus infinite entropy. If one wants entropy over continuous variables to be bounded, then one must work with continuous functions which  \DIFdelbegin \DIFdel{have a }\DIFdelend \DIFaddbegin \DIFadd{maybe have }\DIFaddend very small but still finite support (i.e.~region where the function is non-zero), which is what we will assume.
...
This \DIFdelbegin \DIFdel{should be particularly perplexing if }\DIFdelend \DIFaddbegin \DIFadd{may seem particularly perplexing given that }\DIFaddend we want to use this quantity in a physical setting. If we want $\rho(x)$ to represent the state of a macroscopic system and $H(\rho(x))$ to represent entropy, a state variable, how can $H$ change value if we merely express $\rho$ over different coordinates? How can maximization of entropy be meaningful if it yields different results depending on the coordinate system? Does that mean that we can use the Shannon formula only on distributions over discrete values?

In physics, what makes the Shannon variability work is phase space. What happens is that, under coordinate transformation $\hat{q}^i = \hat{q}^i(q^j)$, the $d\hat{q}^i = \frac{\partial \hat{q}^i}{\partial q^j } dq^j$ vary like vector components while $d\hat{p}_i = \frac{\partial q^j}{\partial \hat{q}^i } dp_j$ vary like covector components. Therefore the areas $d\hat{q}^i d\hat{p}_i = dq^j dp_j$ remain the same. \DIFdelbegin \DIFdel{The Jacobian determinants are unitary and }\DIFdelend \DIFaddbegin \DIFadd{This has a couple of important consequences. First of all, (1) the volumes are preserved during the transformation, which means (2) the Jacobian is unitary and (3) }\DIFaddend the Shannon variability is invariant. \DIFaddbegin \DIFadd{In fact, these three properties are mathematically the same property: each one implies all others. Second, something similar happens to the areas for each independent degree of freedom, meaning that the Shannon variability associated to the marginal distributions is also invariant. These properties can be taken to be the defining characteristics of phase space: phase space has exactly such geometrical properties.\mbox{%DIFAUXCMD
\cite{AoP2020HamiltonianEntropy}
...
%DIF >  http://przyrbwn.icm.edu.pl/APP/PDF/120/a120z6p04.pdf
\DIFaddbegin 

\DIFaddend The Boltzmann constant $k_B$ should not distract us: its role is simply to allow us to measure temperature in an appropriate unit.\DIFaddbegin \footnote{\DIFadd{In the latest revision of the International Metric System of Units SI,\mbox{%DIFAUXCMD
\cite{chyla2011evolution} }\hspace{0pt}%DIFAUXCMD
the Boltzmann constant is one of the defining constants. This means its value is assigned, rather than measured, in a way that properly defines Kelvin, the unit of temperature. The universal constants are essentially used as fixed starting points to define our measurement scales.
}}
\DIFaddend If one defines $T = \frac{\partial U}{\partial S}$, and measures temperature in Kelvin and energy in Joules, the above relationship forces us to measure entropy in Joules/Kelvin. Therefore the entropy cannot be expressed as a pure number. However, this is not the only possible definition. Instead of using $T$ as a primary thermodynamic variable, we can use $\beta = \frac{1}{k_B T}$. \DIFdelbegin \footnote{\DIFdel{There are a few reasons why one can argue this is a better choice, though ultimately it is still a matter of choice.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend In this case, one would define $\beta = \frac{\partial S}{\partial U}$, measure energy in Joules, $\beta$ in inverse Joules and entropy would be dimensionless. As one can do all the thermodynamic calculations using just $\beta$, the constant $k_B$ is really just set by the unit system.\DIFaddbegin \footnote{\DIFadd{As \mbox{%DIFAUXCMD
\cite{chyla2011evolution} }\hspace{0pt}%DIFAUXCMD
notes:
}\begin{quote}
	\DIFadd{Temperature characterizes the average thermal energy of particles in a certain ensemble of particles in the state of thermodynamic equilibrium. Therefore, in principle, one could express this quantity in terms of the unit of energy; this is actually practiced in many fields of physics, especially in statistical physics, where it is quite common to use $\Theta = k_B T$ or $\beta = 1 / k_B T$  as a measure of thermal energy, instead of temperature $T$ measured in Kelvins.
}\end{quote}
}
\DIFaddend 
...
We note that some authors choose to interpret the probability distribution not as coming from repeated independent trials, but as the knowledge one has about the system. This would make the entropy a subjective notion\DIFdelbegin \footnote{\DIFdel{Jaynes\mbox{%DIFAUXCMD
\cite{Jaynes} }\hspace{0pt}%DIFAUXCMD
called it the anthropomorphic nature of entropy.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{, but the }\DIFdelend \DIFaddbegin \DIFadd{: each observer would have a different credence distribution, regardless of whether it fits the data, and therefore a different Shannon entropy. A physical quantity, however, must be the same for everybody, and the }\DIFaddend associated fluctuations we experimentally observe are indeed objective. \DIFaddbegin \DIFadd{So what is going on?
}

\DIFaddend We believe the confusion comes from the, correct, realization that the same system, under different conditions, will be described with a different set of thermodynamic variables. \footnote{Jaynes\cite{Jaynes} points out:
\begin{quote}
...
\end{quote}}
The choice of the system, in statistical mechanics and thermodynamics, is enough to determine the state space for the microstates, but not enough to determine the set of macrostates that correspond to equilibria. We have to specify the process and the constraints that that process puts on the system. Under a different choice of process and constraints the microstates will fluctuate in different ways since we have changed the dynamics of the system. This is what we stated at the beginning: a distribution, and therefore its variability, is always contingent upon some arbitrary choices. \DIFaddbegin \DIFadd{Since the Gibbs entropy is the variability of a microstate within the distribution identified by the macrostate, it is not a property of the single microstate, it is not a property of the system, but it is a property of the system within that specific process, of the macrostate. Some authors refer to this fact by saying that entropy is not objective }\footnote{\DIFadd{Jaynes\mbox{%DIFAUXCMD
\cite{Jaynes,Jaynes2} }\hspace{0pt}%DIFAUXCMD
called it the anthropomorphic nature of entropy.}}\DIFadd{. 
}

\DIFadd{The issue is that subjective, in the context of probability theory, refers to Bayesian probability, which really means subjective: what one believes to be true regardless of what experimental evidence there is. This is not at all the same concept as the one outlined before.}\footnote{\DIFadd{Note how Jaynes\mbox{%DIFAUXCMD
\cite{Jaynes2} }\hspace{0pt}%DIFAUXCMD
always put ``subjective'' in double quotes.}} \DIFaddend The \DIFdelbegin \DIFdel{question, then, is whether the }\DIFdelend system plus the process \DIFdelbegin \DIFdel{plus }\DIFdelend \DIFaddbegin \DIFadd{(which identifies }\DIFaddend the set of constraints\DIFdelbegin \DIFdel{are enough to determine the entropy, or if we also need to mix in human knowledge . What we find is that they are sufficient, so the entropy does not depend on human knowledge. }\DIFdelend \DIFaddbegin \DIFadd{) determines the ensemble and therefore the entropy, regardless of whether an agent knows what process was used. Even if one wants to give a Bayesian account, then, one has to give it in terms of an agent that has full and exact knowledge of the system and the process. But this is hardly subjective. In our view, this is a case of unfortunate word choice. It is much better to simply state that the entropy is a property of the ensemble, of the distribution, and not of the system. And if one understands that the Gibbs entropy is the variability of the microstate within the ensemble, then it is clear that it cannot be a property of the microstate itself, but it is a property of the ensemble.
}\DIFaddend 
...
\DIFadd{We would like to thank Juniar Lucien for insights into the physics education literature. This work is in connection to Assumptions of Physics, a larger project that aims to identify a handful of physical principles from which the basic laws can be rigorously derived  (}\url{https://assumptionsofphysics.org}\DIFadd{)}\DIFaddend .
