...
\end{equation}
where $\sigma_x$ and $\sigma_p$ are the standard deviations of position and momentum and $\Sigma$ is the determinant of the covariance matrix and. The specific value of the uncertainty $\Sigma$ will depend on the entropy and on whether we are using classical or quantum mechanics, though the relationship will always be saturated by Gaussian states with no correlation between $x$ and $p$.
...
\end{axis}
\end{tikzpicture}
    \caption{Entropy $S$ in nats for a Gaussian state as a function of uncertainty $\Sigma$, measured in units of $\hbar$. It shows the effect of the transition from classical to quantum by shrinking the pure states by a factor of $\lambda$ instead of stretching the mixed states by the same factor. The rescaled uncertainty for pure states becomes $\hbar/\lambda$ and the modified Gaussian bound as becomes $S_{\lambda Q}(\Sigma) = S_Q(\lambda \Sigma) - \ln(\lambda)$. Taking $\lambda$ to infinity corresponds to taking $\hbar/\lambda$ to zero.}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.4\textwidth]{stretchedCat.png}
