...
%\geometry{top=4cm, bottom=4cm, left=3cm, right=4cm} %close to actual philsci margins 

%\usepackage{endnotes}
%\let\footnote=\endnote %Erkenntnis requires end-notes, but recent papers don't seem to follow this in the pdf!
 % %To call the endnotes, use command \theendnotes before calling reference page 
...
\titlespacing*{\section}{0pt}{2.5mm}{2.5mm}%spacing from left margin, spacing above header, spacing on right of header
\fi
...
\pagestyle{fancy}
\fancyhead{}
\fancyhead[R]{\thepage} %page number placement
\fancyhead[C]{\textsc{Hamiltonian Privilege}}
%\fancyhead[EC]{\textsc{AUTHOR NAMES}}%Even page header 
\fancyfoot[L,R,C]{}
...
\usepackage[dvipsnames]{xcolor}%lets us define a new color 
...
@Unpublished{Butterfield2004,
  author  = {Butterfield, Jeremy N.},
  Title   = {Between Laws and Models: {S}ome Philosophical Morals of {L}agrangian Mechanics},
  Year    = {2004},
  Address = {https://arxiv.org/abs/physics/0409030},
}

@InCollection{Butterfield2004a,
  Title     = {On {H}amilton--{J}acobi Theory as a Classical Root of Quantum Theory},
  Publisher = {Springer},
  Year      = {2004},
  author  = {Butterfield, Jeremy N.},
  Editor    = {Elitzur, A. C. and Dolev, S. and Kolenda, N.},
  Address   = {New York},
  Booktitle = {Quo Vadis Quantum Mechanics? {P}ossible Developments in Quantum Theory in the 21st Century},
}
...
@Unpublished{Swanson,
	...
	journal = {},
	doi = {},
	address = {http://philsci-archive.pitt.edu/9314/}, 
}
...
@Unpublished{Baez,
  ...
  address = {https://arxiv.org/abs/1609.01421}, 
}
...
@Article{Gilton2021,
  ...
  Title     = {Could Charge and Mass be Universals?},
  Journal   = {Philosophy and Phenomenological Research},
  Year      = {2021},
  Volume    = {102},
  Pages     = {624--644},
  Publisher = {Wiley Online Library},
Url = {https://doi.org/10.1111/phpr.12681}, 
}
...
\usepackage[backend=bibtex, authordate, url=false, doi=false, eprint=false]{biblatex-chicago}
%\addbibresource{bibliography.bib}
\addbibresource{references.bib} %call the bib file generated above 
...
\AtEveryBibitem{%
  ...
  \clearfield{pagetotal}%
   \clearfield{chapter}%
  \clearfield{number}%for convenience/consistency
}
...
\hypersetup{
	pdftitle={Hamiltonian Privilege},
	pdfauthor={},
	pdfsubject={},
	pdfkeywords={},
	pdfcreator={XeLaTeX},
...
%provided a theoretical framework captures the relevant behavior, it is more fundamental if it requires fewer conditions to hold. 
...
When the system is characterized by continuous quantities, {\color{green}$\mathcal{S}$ can be taken to be a manifold}.\footnote{{\color{green}The use of manifolds is, technically, a choice of how the physics is represented. One could make another choice, for example, by representing the state variables not as local coordinates but as an algebra of operators that act on the state. In this case, we conjecture that a similar argument would demonstrate an analogue of a symplectic structure for the different mathematical representation.}} For $U \subseteq \mathcal{S}$, we define $\mu(U) := \int_U \rho_c(s) d\mathcal{S}$ to measure the number of particles in region $U$. \textit{State variables} $\xi^a$ are any set of quantities that fully identify states, so that $\xi$: $\mathcal{S} \to \mathbb{R}^n$ is an invertible function between each state and the numeric values of the variables.\footnote{In the language of manifolds, these $\xi^a$ are called ``coordinates'' and $\xi$ is a coordinate map. To avoid confusion, we will call the coordinates of the state space $\mathcal{S}$ ``state variables.'' As usual with coordinate charts, $\xi$ is properly defined on some $U \subseteq \mathcal{S}$, and $\xi^{-1}$ is defined on $\xi(U) \subseteq \mathbb{R}^n$. We will neglect these subtleties in what follows.} The map $\xi$ represents a state $s \in \mathcal{S}$ by a set of state variables $\xi^a \in \mathbb{R}^n$, such that $\xi (s) = \xi^a$. To represent the distribution $\rho_c$ in terms of state variables, we define a density $\rho_{c, \xi}$: $\mathbb{R}^n \to \mathbb{R}$ by $\rho_{c, \xi} \equiv (\rho_c \circ \xi^{-1})$. This density plays a crucial role throughout our argument. It allows us to compare and track the amount of matter in different possible states.
...
%Since, the only requirement that we have imposed on the Hamiltonian is that the equations yield a unique solution, our derivation applies to Hamiltonians that are not hyper-regular. }

% since we have not imposed hyper-regularity,   }
...
%; otherwise, the positions and velocities do not determine how the system evolves 
...
%\footnote{We provide details of the quantum and statistical mechanical cases elsewhere. Moreover, we show that Hamiltonian mechanics provides the proper setting for the invariant density and a simple understanding of the gauge-dependency of conjugate momentum.}  
...
\setlength\bibitemsep{2pt} %controls spacing between bib entries. default is itemsep 
