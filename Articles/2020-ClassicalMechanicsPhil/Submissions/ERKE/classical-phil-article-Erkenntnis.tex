...
\noindent
This assumption characterizes classical systems as those whose infinitesimal parts completely specify the state of the system. We define a \textit{classical particle} as one of these infinitesimal parts, namely the limit of recursive subdivision.\footnote{This definition of a classical particle differs from viewing point-particles as extensionless \parencites[]{Butterfieldpoints} or zero-dimensional \parencites[]{Wilson}.} The general idea is that this assumption tells us that the state of a system is captured by a statistical distribution over the state space of the parts. If the state space of the parts is a manifold, this must be differentiable so that it can support a density at each point. Moreover, given that this density must be a function of the state, and not of the coordinates, we find that the manifold must be symplectic.
...
\noindent In our case, this applies to both the state of the whole system and the state of its parts. Under this assumption we will find the symplectic structure must be preserved in time, which gives us Hamiltonian evolution. 
