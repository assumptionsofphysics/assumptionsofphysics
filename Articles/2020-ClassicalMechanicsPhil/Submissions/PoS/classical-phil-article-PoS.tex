...
\fancyhead[C]{\textsc{HAMILTONIAN PRIVILEGE}}% odd page header 
...
\newif\iffootpunct %defines \footpunctfalse  and \footpuncttrue commands

% % Essential to modify footnote ONLY AFTER BIBLATEX COMMANDS, since otherwise ruins citation styles 

% % footnote modification: must be between \makeatletter and \makeatother
 \makeatletter
  
 \def\blfootnote{\gdef\@thefnmark{} \@footnotetext} %footnote without number marker
  ...
  \begin{list}{}{\setlength{\labelwidth}{0mm}% indents footnote text, Space from number to border
    ...
    \setlength{\labelsep}{0pt}% Space from number to text, moves footnote mark to the left
     \setlength{\itemsep}{0pt}%
      ...
      }%\doublespacing \normalsize% double spaces and changes font size of footnotes; place before }
  \item[\@thefnmark \iffootpunct. \fi \hfil ]#1% @makefnmark, with period iff footpuncttrue
  \end{list}%
}
  

%      \setlength{\footnotesep}{0.83125\baselineskip}% separation between footnotes 
...
\date{\vspace{-7.9ex} July 30, 2021}
\maketitle
%
\vspace{-8mm}
...
\footpunctfalse% turns off period after footnote marker for blindfootnote

%\blfootnote{*To contact the author, please write to: }
%\blfootnote{\textdagger For comments on earlier drafts, we thank }
