...
\subsection{Density and Riemmanian geometry}

It is helpful to take a quick look at how densities work in general so that we can better understand what is so special about them in the context in Hamiltonian mechanics. By density we mean the intuitive physical notion of a quantity divided by the region it occupies, like mass per volume or charge per area. Over a continuum, this is an integrable function $\rho(x)$. Mathematically, the concept is captured by the Radon–Nikodym derivative: we have a region $U$, we have two measures $m$ and $V$ and the derivative satistifies $m(U) = \int_U \rho dV$. This makes it clear that the units of $\rho$ depends on the units of both $m$ and $V$. More precisely, if $\rho(x, y)$ is a function of multiple variables, we will want $\int_U \rho(x, y) dx dy$ so that the units of $\rho$ are consistent with the units of the arguments. For example, in polar coordinates $\rho(r, \theta)$ will return mass over meter-radians if we measure $r$ in meters and $\theta$ in radians.

The density is coordinate dependent because its definition requires us to make a choice of coordinates. Worse than that, it is not even coordinate invariant: the density changes with the Jacobian. For example, changing from polar to Cartesian coordinates we have $\rho(x,y) = |J| \rho(r, \theta) = r \rho(r, \theta)$. Since the Jacobian is a function of the point, this makes the value of the density, in general, not directly comparable at different points. If we have a uniform density in $x$ and $y$, we will have an $r$-dependence in $r$ and $\theta$. This dependence is introduced precisely because the area identified by $dr$ and $d\theta$ is $dr \,r d\theta$, which is a function of $r$.

In differential geometry, a different mathematical object is called the `density,' namely a volume form (i.e. an alternating function from $n$ vectors to a real number) that is unaffected by changes in orientation. Our physically significant density $\rho$ is actually the component of this volume form $\nu = \rho(x^i) dx^1 \wedge dx^2 \wedge ... \wedge dx^n$. Note that $\nu$ combines the density and the volume into a coordinate-free object. Considering the total mass in the region $U$, we have $m(U) = \int \nu$ so essentially $\nu = dm$ which is the infinitesimal mass contribution we are integrating. But this means the value returned by $\nu$ depends not only on how must stuff there is at a given point, but also on the size of the volume identified by the $n$ vectors given to the form. [Not immediately clear to me what the upshot is of this last point; e.g. how does this relate to $\nu$ being coordinate-free?]

In Riemannian geometry, we have a canonical volume form $dV = \epsilon \sqrt{|g_{ab}|} dx^1 \wedge dx^2 \wedge ... \wedge dx^n$ where $|g_{ab}|$ is the determinant of the metric tensor as expressed in local coordinates, and $\epsilon$ is $1$ or $-1$ depending on the orientation. In component notation, we can express the volume form as $\sqrt{|g_{ab}|}\epsilon_{a_1 ... a_n}$, where $\epsilon_{a_1 ... a_n}$ is the Levi-Civita symbol. We can then express any differential density $\nu = \hat{\rho} dV$ as the product of the volume density times a scalar function $\hat{\rho}$. At first glance, this expression misleadingly suggests that the density $\nu$ is coordinate-independent, but this is not the case. [double check that this claim applies to $\nu$ rather than $\hat{\rho} $. Is this in tension with saying earlier that $\nu$ is coordinate-free?] The quantity $|g_{ab}|$ depends on the choice of coordinates and is, in general, a function of the point. However, we can always find a set of coordinates that, at least locally, makes $|g_{ab}|$ constant. The determinant $|g_{ab}|$ is the Jacobian of the transformation to those coordinates [the rationale/point of this sentence was not clear to me]. For example, in Cartesian coordinates $|g_{ab}| = 1$ and the volume form $dV = dx \wedge dy$; in polar coordinates $|g_{ab}| = r^2$ and the volume form $dV = r dr \wedge d\theta$ [above we said that the Jacobian was $r$ rather than $r^2$]. This means that $\hat{\rho}$ is really expressing the density in a particularly convenient Cartesian-like coordinate system: one in which we can compare the density point-wise. We can also see this if we look at the units of $\hat{\rho}$, which are in terms of inverse volume as expressed in the privileged system.\footnote{This is similar to what happens with the concept of proper time: it is defined in the comoving frame, so it is coordinate dependent. Yet, the expression $d\tau = \frac{1}{c} \sqrt{g_{ab} dx^a dx^b}$ is coordinate invariant.}

We can now look at what happens for distributions over position and velocity. Note that the velocities transform in the same way (i.e. contravariantly) as differentials in position. Therefore $\rho(x^i, v^j) = \sqrt{|g_{ab}|} \sqrt{|g_{ab}|} \rho(\hat{x}^i, \hat{v}^j) = |g_{ab}| \rho(\hat{x}^i, \hat{v}^j)$ $dx^a$. The determinant of the metric tensor is therefore directly the volume of the tangent bundle [I didn't follow this point]. The relationship to the density as expressed in phase space is therefore the following:
\begin{equation}
	\rho(q^i, p_j) = |g_{ab}| \rho(x^i, v^j).
\end{equation}
...
% Also remember: there are quantities that are both coordinate-invariant and coordinate -dependent that are still physically significant, e.g. choices of q's and p's (?)
...
Understanding both mechanics in terms of our assumptions gives us a way to understand gauge transformations that is both more physically grounded and intuitive. Recall that motion equivalence entails that knowledge of the trajectory in physical space suffices for knowledge of the state in phase space. In other words, we can completely characterize states in terms of the canonical position coordinate $q$ and its first derivative $\dot{q}$. Hence, any transformation that leaves $(q,\dot{q}) $ invariant \textit{ipso facto} leaves the physical state invariant. We want such transformation to be canonical as to preserve the invariant densities. This means that the new state variable $(q, \hat{p})$ will respect the usual Poisson brackets:
\begin{equation}
\begin{aligned}
	\{q^i, q^j\} &= 0 \\
...
\end{aligned}
\end{equation}
Since $A_j$ is curl free, we can express it as the gradient of a function $f$. Therefore changes of state variable of the form $\hat{p}_j = p_j + \partial_j f$ do not change either the equations of motion or the trajectories. We recognize these as the gauge transformations.
