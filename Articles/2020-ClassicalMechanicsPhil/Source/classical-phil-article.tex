...
We begin with the simplest case: suppose that a single coordinate $q $ suffices to define the unit system for the particle state space. Then, the state variables are given by a set $\xi^a = \{ q, k^i \}$, where \textit{prima facie} the index $i$ can range over more than one number. [XXX: or less] We will show that, in fact, there can only be a single $k^i$ variable in this case, conjugate to the single $q$ coordinate. Assuming that the particle density $\rho_c$ is a scalar, it follows that we can arbitrarily change the unit defined by $q$ without changing the density $\rho_c$. 
...
The conclusion is that, under the infinitesimal reducibility assumption, a state $c$ of the system is a coordinate-invariant distribution over the state space $\mathcal{S}$ of its infinitesimal parts. If the state space of the particles is a manifold, then it must be a symplectic manifold: symplectic manifold structure is necessary for the distribution to be coordinate-invariant. Additionally, any symplectic manifold structure will allow the definition of coordinate--invariant distributions: any scalar function can be in fact integrated using the symplectic form in a coordinate-invariant way. Therefore the symplectic structure is exactly the structure needed for their definition. [XXX:]
...
Note that weak equivalence is not enough. The classical state, as we defined it, is really the composite, the distribution, and we must be able to express that distribution in terms of the position and velocity. [XXX] To have full equivalence, we must be able to express the density as well. Therefore we must have $\rho(q^i, p_j) = |J| \rho(x^i, v^j) = \left|\frac{\partial v^i}{\partial p_j}\right| \rho(x^i, v^j)$ since
\begin{equation}
\begin{aligned}
|J| &= \begin{vmatrix}
