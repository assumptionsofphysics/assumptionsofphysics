...
%[XXXJ: if previously we measured the density in particles per meter cube---and we change units from meters to miles---won't the units of the density change to particles per cubic mile? Isn't this required by dimensional analysis on the expression $\int_U \rho^{\ast}_c(\xi^a) d\xi^a = \int_U \rho^{\ast}_c(\xi^b) d\xi^b$ ?].
