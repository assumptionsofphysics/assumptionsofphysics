...
%For discussion of the status of extensionless point particles within classical mechanics, see [have yet to find decent reference for this kind of discussion]. 
...
In the continuous case, when $\mathcal{S}$ is a manifold, we define $\mu(U) = \int_U \rho_c(s) d\mathcal{S} $. We will call a set of quantities $\xi^a$ \textit{state variables} provided that they fully identify a state, so that $\xi$: $\mathcal{S} \to \mathbb{R}^n$ is an invertible function between each state and the numeric values of the variables.\footnote{In the language of manifolds, these $\xi^a$ are called `coordinates'. However, we will have multiple manifolds and, to avoid confusion, we will call `state variables' the coordinates of the state space.} We define a density $\rho_{c, \xi}$: $\mathbb{R}^n \to \mathbb{R}$ as the pullback of the distribution $\rho_c$ by the inverse of a coordinate map:  $\rho_{c,\xi} \equiv (\rho_c \circ \xi^{-1})$. %To represent the distribution over state variables, we define coordinate charts $(U, \xi)$ over $\mathcal{S}$. Likewise, the distribution $\rho_c$ is the pullback of $\rho^{\ast}_c$ by a coordinate map: $\rho_c (s) = (\xi^{\ast} \rho^{\ast}_c)(s) = \rho^{\ast}_c (\xi (s))$. 

The distribution $\rho_c$ is a function of the particle states $s \in \mathcal{S}$, while the density $\rho_{c, \xi}$ is a function of the state variables $\xi^a \in \mathbb{R}^n$. A coordinate map $\xi$ lets us express the state $s$ as a function of the coordinates $\xi^a$. Consequently, we require that $\mu(U) = \int_U \rho_c(s) d\mathcal{S} = \int_{\xi(U)} \rho_{c, \xi} (\xi^a) d \xi^a$.\footnote{Indeed, using the Lebesgue measure $\lambda$ on $\mathbb{R}^n$---and assuming that $\xi^{-1}$: $\mathbb{R}^n \to \mathcal{S}$ is measurable---we can define the measure $d\mathcal{S}$ by the pushforward measure $\xi^{-1}_{\ast}(\lambda)(U) \equiv \lambda(\xi(U))$.} Since $\rho_c$: $\mathcal{S} \to \mathbb{R}$ is a function of the particle state $s$, its value is independent of the choice of state variables $\xi^a$. Hence, $\rho_{c, \xi}$ will transform as a scalar under change of variables. Thus, $\int_U \rho_c(s) d\mathcal{S}$ is invariant under coordinate changes $\xi$ to $\hat{\xi}$. As a function of the state variables $\xi^a$, $\rho^{\ast}_c$ transforms according to the Jacobian for this change of variables: $\rho^{\ast}_c (\xi^a) = \left|\frac{\partial \hat{\xi}^b}{\partial \xi^a} \right| \rho^{\ast}_c (\hat{\xi}^b)$. Since $\mu(U) = \int_{\xi(U)} \rho^{\ast}_c (\xi^a) d \xi^a = \int_{\hat{\xi}(U)} \rho^{\ast}_c (\hat{\xi}^b) d \hat{\xi}^b$ remains invariant under this coordinate change, the Jacobian determinant must equal one. By dimensional homogeneity, it must also be dimensionless.\footnote{Formally, we can express this argument as a chain of equalities: $\rho^{\ast}_c(\hat{\xi}^b)=\rho_c(s(\hat{\xi}^b))=\rho_c(s(\xi^a))=\rho^{\ast}_c(\xi^a) = \left|\frac{\partial \hat{\xi}^b}{\partial \xi^a} \right| \rho^{\ast}_c(\hat\xi^b)$, where $\rho^{\ast}_c(\xi^a)$ and $\rho^{\ast}_c(\hat{\xi}^b)$ are the same density expressed over different state variables and $s(\xi^a) \equiv \xi^{-1}(\xi^a)$ and $s(\hat{\xi}^b)\equiv \hat{\xi}^{-1}(\hat{\xi}^b)$ represent the same particle state as expressed by these different variables. Note that $\rho_c(s(\hat{\xi}^b))$ shows how the distribution depends on the state variables only through their labeling of a particle state $s$.} 
%$\rho^{\ast}_c(\hat{\xi}^b)=\rho_c(s(\hat{\xi}^b))=\rho_c(s(\xi^a))=\rho^{\ast}_c(\xi^a) = \left|\frac{\partial \hat{\xi}^b}{\partial \xi^a} \right| \rho^{\ast}_c(\hat\xi^b)$

We will now show that in order for the distribution $\rho_c$ to satisfy these two constraints, the classical phase space must be at least a symplectic manifold. In other words, having symplectic manifold structure is a necessary condition for the distribution over particle states to transform as a scalar function and for its pullback $\rho^{\ast}_c := \rho_c \circ \xi^{-1}$ to transform as a density.

%In other words, while we can change the units used by the state variables $\xi^a$ to identify the particle states, the units for $\rho^{\ast}_c$ must remain the same so that the density does not change its numeric value
...
%[XXJ: if previously we measured the density in particles per meter cube---and we change units from meters to miles---won't the units of the density change to particles per cubic mile? Isn't this required by dimensional analysis on the expression $\int_U \rho^{\ast}_c(\xi^a) d\xi^a = \int_U \rho^{\ast}_c(\xi^b) d\xi^b$ ?].
...
In terms of physics, then, a set of canonical coordinates is a set of state variables that express the density in the correct units. A canonical transformation is one that preserves the unit and the numerical value of the density. A change of unit is a special type of canonical transformation: one where we have an arbitrary change in the unit variables and an induced (covariant) change in the conjugate variables.
...
%The gauge-dependency of the conjugate momentum supplies further grounds for privileging the invariant phase space density (as the fundamental description of a classical state)? 
...
% % [To do: think about whether treating gauge-invariance as a necessary condition for fundamentality is in tension with our position, specifically our contention that coordinate-dependency does NOT preclude a quantity from fundamental physical significance. Does our position require an important physical difference between gauge-dependency and coordinate-dependence?]. Maybe the thought is that gauge-invariance and coordinate-invariance are on the same footing. So if coordinate-invariance is a necessary condition for physical significance, then so is gauge-invariance. A gauge-dependent quantity can still be physically significant, in the same way that a coordinate -dependent quantity can be physically significant.
% % Note that if coherent, our position has the following consequence: gauge-dependency is a sufficient condition for a quantity to be non-fundamental (in the same way that coordinate-variance is sufficient for non-fundamentality). But coordinate-dependency is NOT a sufficient condition for being non-fundamental. This could strike some readers as in tension. If we are comfortable viewing gauge-dependency as precluding a quantity from fundamental significance, then why not coordinate-dependence? Response: we just need to motivate that gauge-dependency is more analogous to coordinate-variance than coordinate-dependence.
...
Treating time $t$ as both a coordinate and the parameter of the trajectory, the time component of the displacement, $S_t = dt/ dt = 1$.\footnote{Considering $dt/dt$, the time variable in the numerator functions as a coordinate, while the time variable in the denominator functions as a parameter. We could instead use a parameter $\tau$, and the derivation would remain valid.} This places another constraint on $\vector{\theta} $, since $\vector{S} = -curl(\vector{\theta}) $. In components, this means that $S_t = - (\frac{\partial}{\partial q} \cdot \theta_p - \frac{\partial}{\partial p} \cdot \theta_q) = - (\frac{\partial}{\partial q} \cdot 0 - \frac{\partial}{\partial p} \cdot \theta_q) = \frac{\partial q}{\partial p}$. Hence, $S_t = 1$ entails that $\frac{\partial q}{\partial p} = 1$. Integrating this expression, we find that $\theta_q = p$ up to addition by an arbitrary constant function $g(q)$. Again using the gauge freedom of $\vector{S}$ we can set $g(q)=0$, so that $\theta_q = p$. Thus, so far, we have shown that $\vector{\theta} = (p, 0,\theta_t) $.
...
Nonetheless, reformulating Hamiltonian mechanics within the extended phase space $(q_i, p_i, t, E)$ provides one way to rectify these deficiencies. At root, the time-dependence of the Hamiltonian stems from the dual role of time as both a coordinate and a time-like parameter within the usual Hamiltonian framework. Yet, differential geometry demands that we separate these two roles. We thereby add a temporal degree of freedom $(t, E) $, expanding the phase space degrees of freedom by one (recall that we previously had one degree of freedom for every canonical coordinate pair $q_i, p_i$). Next, we define the phase space evolution over an evolution parameter $s $. This yields two more equations of motion: $dt/ds = -\partial \mathcal{H}/ \partial E$ and $dE/ds = -\partial \mathcal{H}/ \partial t$, where $\mathcal{H}(q_i, p_i, t, E)$ is the extended Hamiltonian. The extended Hamiltonian not only acts as a generator of the evolution parameter $s$, but it also provides us with the \textit{Hamiltonian constraint} $\mathcal{H}=0$ [XXX to do: provide a reference for extened phase space formulation. And check this discussion of the `Hamiltonian constraint']. The Hamiltonian constraint provides the relationship between the energy and the other state variables. Crucially, the extended Hamiltonian---unlike the ordinary Hamiltonian---is coordinate-invariant, even with respect to time-dependent coordinate transformations. For example, if we define the affine parameter as the proper time, then the extended Hamiltonian is proportional to the mass (which is manifestly coordinate-invariant). This provides a clear physical meaning for the extended Hamiltonian. We have now considered many conceptual and physical advantages of the Hamiltonian formalism. In the next section, we rebut Curiel's \parencites*[]{Curiel} arguments against privileging Hamiltonian mechanics. 
%%Arnold uses a related extended phase space $(q_i, p_i, t)$ in his chapter 9, but doesn't provide a general discussion of it. 
%Discussion by GC of extended phase space: https://tinyurl.com/4tlbej76
