...
We can now look at what happens for distributions over position and velocity, i.e. on the tangent bundle in Lagrangian mechanics. Note that the velocities transform in the same way (i.e. contravariantly) as differentials in position. Consequently, the density $\rho(q,\dot{q})$ also varies contravariantly. These transformation properties preclude the density---in the Lagrangian formalism---from being coordinate-invariant in general. In detail, $\rho(x^i, v^j) = \sqrt{|g_{ab}|} \sqrt{|g_{ab}|} \rho(\hat{x}^i, \hat{v}^j) = |g_{ab}| \rho(\hat{x}^i, \hat{v}^j)$. The determinant of the metric tensor is therefore directly the volume of the tangent bundle. The relationship to the density as expressed in phase space is therefore the following:
\begin{equation}
...
% Josh: why is the determinant of the metric tensor equal to the volume of the tangent bundle in this case? [Discussed on 1-4 meeting, 11:56 a.m.]. Perhaps study discussion of this in the long paper.
