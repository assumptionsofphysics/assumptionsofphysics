...
% % What we really want is a brief formalization of ``proper subset," e.g. using partitions of possible worlds semantics. See if I can find Lewis discussion of this (maybe mentioned by Plebani)
% % There is perhaps a further difficulty about the identity criteria for counting conditions. So we really just need it to be the case that all of the Lagrangian systems are a subject matter subset of the Hamiltonian systems. E.g. in the sense that the 1800s is a subset of all of history. need a mereological relationship. 
% Perhaps could say: this notion of a proper subset can be spelled out using Lewis on subject matter. Butterfield/gomes paper. But I could not find this discussion in their paper (might have a false memory)
% % we could formalize this notion of ``proper subset" using what Lewis does regarding the mereology of partitions of possible worlds. This gives an account of subject matters, similar to Yablo's. 
...
%The property of infinitesimal reducibility provides a fruitful notion of a classical system. Classical systems are infinitesimally reducible systems. Hence, we feel this is the proper notion of a classical system.
...
As indicated at the start of Section~\ref{privileging}, we use ``fundamentality" in a non-metaphysical fashion. Differences in fundamentality are primarily differences in generality: more fundamental frameworks require fewer conditions to hold.\footnote{We also embrace related non-metaphysical uses of ``fundamental," discussed in Section~\ref{fundamental}.} Our notion of fundamentality leads to a correspondingly non-metaphysical notion of ``structure." Hamiltonian mechanics employs less structure than Lagrangian mechanics because the former requires fewer physical conditions. Compared with North's \parencites*[]{North} account of structure, our definition has two advantages. First, it avoids metaphysical disputes surrounding perfectly natural or joint-carving properties, which empiricists and pragmatists decry. Secondly, it avoids a technical objection that Swanson and Halvorson \parencites*[]{Swanson} levy against North's more precise mathematical criterion for comparing structure. 
...
% A state variable change $\hat{\xi}$ represents the same state $s$ by a different set of state variables $\hat{\xi}^b$. 
...
%footnote{Indeed, using the Lebesgue measure $\lambda$ on $\mathbb{R}^n$---and assuming that $\xi^{-1}$: $\mathbb{R}^n \to \mathcal{S}$ is measurable---we can define the measure $d\mathcal{S}$ by the pushforward measure $\xi^{-1}_{\ast}(\lambda)(U) \equiv \lambda(\xi(U))$.} 
...
% % perhaps put at end of section 5.1: We have just shown that Hamiltonian mechanics provides the proper setting for representing the density. By representing the density on a symplectic manifold, we do not need to know which coordinate system we have chosen to work in.
...
At this point, we return to the action $ \mathscr{A}$. If the motion equivalence condition holds, then $ \mathscr{A}$ is defined as the integral of the Lagrangian over a trajectory beginning at time $t_0$ and ending at time $t_1$: $\integral^{t_1}_{t_0} L (q, \dot{q}, t) dt$. If motion equivalence does not hold, we can define the action more generally over the function $L_p \equiv p \dot{q} - H(q, p, t)$, considering paths in the extended phase space. By deriving the stationary action principle in this more general setting, we simultaneously derive its usual version within Lagrangian mechanics.\footnote{When motion equivalence holds, the Legendre transform is well-defined, allowing us to express $L_p$ as a function of $(q, \dot{q}, t)$, i.e. $L_p = L(q, \dot{q}, t) = p \dot{q} - H$.} We have then that $ \mathscr{A}[\gamma] = \integral^{t_1}_{t_0} L_p (q, p, t) dt = \integral^{t_1}_{t_0} [p \frac{dq}{dt} - H(q, p, t) ]dt$, where $\gamma$ is an actual trajectory followed by the system. Multiplying the Hamiltonian by $dt/dt = 1$ and adding a vanishing $0 \cdot \frac{dp}{dt}$, we obtain $ \mathscr{A}[\gamma] = \integral^{t_1}_{t_0} [p \frac{dq}{dt} + 0 \cdot \frac{dp}{dt} - H(q, p, t) \frac{dt}{dt} ]dt$. We notice that the integrand simply equals the inner product of $\vector{\theta}$ with $\vector{S} $, entailing that $L_p =\vector{\theta} \cdot \vector{S} $. Consequently, the action $ \mathscr{A} [\gamma] = \integral^{t_1}_{t_0} \vector{\theta} \cdot \vector{S}  dt$. Note that $\vector{S}  dt = (\frac{d q }{d t } dt, \frac{d p }{d t } dt, \frac{d t }{d t }dt ) = (dq, dp, dt) = d\vector{\gamma}$ represents the infinitesimal displacement along the trajectory. Therefore, the action equals the line integral of $\vector{\theta}$ along the trajectory $\gamma $: $ \mathscr{A} [\gamma] =\integral_{\gamma} \vector{\theta} \cdot d\vector{\gamma}$. To derive Hamilton's principle of stationary action, it suffices to show that $\delta \mathscr{A} [\gamma] =\delta \integral_{\gamma} \vector{\theta} \cdot d\vector{\gamma} = 0$. 
  %Provided that the Legendre transform is well-defined (i.e. the motion equivalence assumption is satisfied), then $H = p \dot{q} - L$. Hence, we can re-express the action as $\integral^{t_1}_{t_0} L (q, \dot{q}, t) dt = \integral^{t_1}_{t_0} [p \frac{dq}{dt} - H(q, p, t) ]dt$.
...
%(XXX to do (might no longer be necessary given earlier and later remarks) Note how the geometry is better understood in extended phase space, requiring the Hamiltonian picture. Note that this applies even when $\theta \cdot S$ cannot be expressed as a function of $q$ and $\dot q$. )
