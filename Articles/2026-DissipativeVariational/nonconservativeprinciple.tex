...
\begin{condition}[Classical states]{CST}
	The state space of a classical system with $n$ independent degrees of freedom (DOF) is represented by the symplectic manifold $(\mathbb{R}^{2n}, \omega)$. Each state is represented by position and momentum $(q^i,p_j) \in \mathbb{R}^{2n}$ and $\omega = \sum_i dq^i \wedge dp_i$. The count of configurations\footnote{By \emph{configuration} we mean the position-momentum pair for a single DOF.} for a DOF is given by the $\int_{\Sigma} \omega = \int_{\Sigma} \sum_i dq^i dp_i$. The count of states is given by the Liouville measure $\int_U \omega^n / n! = \int_U \prod_i dq^i dp_i$. All geometric objects are assumed smooth.
\end{condition}
...
\begin{theorem}
	...
	\begin{description}
		...
		\item [TE-FLUX] (Evolution kills the flux) The evolution kills the flux of configuration. That is, there exists a configuration flux form $w_{ab}$ such that $d_t \gamma^a(t) w_{ab}=0$;
		\item [TE-STFL] (Principle of stationary flux) The evolution between two given positions is the one that renders the flux around it stationary. That is, there exists a flux functional $\phi$ such that $\delta \phi[\Sigma_{\gamma, \epsilon \eta}]=0$ where $\Sigma_{\gamma, \epsilon \eta}$ is the surface between the path and its variation.
		\item [DI-SYMP] (Symplectomorphism) The evolution preserves the configuration count and DOF independence. That is, $d_t \omega = 0$.
		\item [DI-STACH] (Principle of stationary action in phase space) The evolution between two given positions is the one for which the phase space action is stationary. That is, there exists an action functional $\mathcal{A}_H[\gamma] = \int (p_i d_t q^i -H)dt$ such that  $\delta \mathcal{A}_H[\gamma] = 0$.
		...
		\item [DI+KE-STAC] (Principle of stationary action) The evolution between two given positions is the one for which the action is stationary. That is, there exists an action functional $\mathcal{A}_L[\gamma^{q^i}] = \int L(x^i, v^i, t)dt$ with $|\partial_{v^i} \partial_{v^j} L| \neq 0$ such that  $\delta \mathcal{A}_L[\gamma^{q^i}] = 0$.
	\end{description}
