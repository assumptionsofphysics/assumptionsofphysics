...
TODO: note that we can write a "Lagrangian type expression" by contracting $w_{ab}d_t\gamma^b$. This gives us a one form instead of a scalar function like the Lagrangian, but we could express it based on the decomposition in one DOF.
...
In true spirit of Reverse Physics, we found five equivalent conditions
\begin{equation}
\begin{aligned}
    d_t \gamma^a(t) = S^a &\iff d_t \gamma^a(t) w_{ab}=0 \iff \delta \phi[\Sigma_{\gamma, \epsilon \eta}]=0 \\&\stackrel{\text{DR}}{\iff} \delta \mathcal{A}_H[\gamma + \epsilon \eta] = 0 \stackrel{\text{DR+KE}}{\iff} \delta \mathcal{A}_L[q(t) + \epsilon \eta^q(t)] = 0.
\end{aligned}
\end{equation}
