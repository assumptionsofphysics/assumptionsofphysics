...
Now, the above is only a partial solution that solves some problems and opens others. But the result stands, and it start giving us some insight in why the problem exists and how, in a final solution, should be solved. The topology must be connected to observable and the definition of the state must include their Lie algebra.
...
\textbf{Applicability of unitary evolution.} The above results make it clear that the Schrodinger equation applies, and only applies, to deterministic and reversible systems: when the evolution of the system depends, and only depends, on itself. Any open evolution is necessarily not unitary, and a measurement is necessarily not an open evolution since the system must interact with the probe.
