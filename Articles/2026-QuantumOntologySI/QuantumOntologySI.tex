...
\begin{Theorem}
	...
	\begin{description}
		\item[HM-1D] (Hamiltonian mechanics) The equations of motion are given by $d_t q = \partial_p H$ and $d_t p = - \partial_p H$.
...
\end{quote}
The core problem was that he could not see a good physical justification for them. One can argue that the choice still made sense back then, because Hilbert spaces were the best functional analysis tools at the time. Topological vector spaces, the most prominent foundational tool for modern functional analysis, arrived later and were, in a sense, retrofitted by Gelfand through rigged Hilbert spaces. While these provides better tools for calculation, states are still element of a Hilbert space, therefore they do not solve the conceptual issue. In other words, the choice of Hilbert spaces is mostly a historical accident, that ignores almost a century of advances in functional analysis.

In finite dimensions, there is only one vector space topology and all linear functionals are continuous. But, as Aliprantis and Border state [cite Hitchhiker's guide]
\begin{quote}
...
The use of Hilbert spaces can be characterized as
\begin{condition}[Quantum states on Hilbert space]{QST:H}
	The state space of the system is represented by the projective space $P(\mathcal{H})$ of a complex Hilbert space $\mathcal{H}$. Observables are represented by self-adjoint operators,
\end{condition}
which lead to
...
\textbf{A more physical definition.} A full solution to the mathematical problems is not just out of scope of this work, but it is also not possible if we do not understand what the physical requirements. The issue, that often confuses philosophers and mathematicians, is that in physics we relax requirements depending on the problem at hand. For example, we typically think of a classical Hamiltonian as a nice, smooth function defined on all phase space. However, we also use Coulomb potentials, which diverge at the origin. Similarly, we think of mass and charge density distribution as smooth functions of physical space. However, we also use point particles, which correspond to infinite densities. Therefore, we already know we are never going to have a single set of conditions that are always valid, this is not how physics works. What we need to find is a set of reasonable condition for the standard case, so that we know when and how we are violating them with a further approximations. The general goal, then, is force everything to be explicit.

Let us give, then, a tentative definition of a notion of a vector space that works better for quantum systems.
\begin{Definition}
	...
	\end{itemize}
\end{Definition}
We still have a complex vector space, with a topology and an inner product that is continuous in that topology. However, observables must be defined for all states and must be continuous, which addresses both \ref{[B]COBS} and \ref{[B]WDQ}. Moreover, a set of defining observables is specified, together with their commutation relationships. This is something we do anyway in physics, so it seems harmless, but it actually solves a couple of different problems. First of all, it solves \ref{[B]STDIST}. Consider two Schwartz spaces $\mathcal{S}(\mathbb{R})$ and $\mathcal{S}(\mathbb{R}^n)$. These are still isomorphic, so, even if all polynomials of position and momentum are continuous, the spaces are isomorphic as topological vector spaces. However, if we add the Lie algebra of the commutators $\frac{[X^i, P_j]}{\imath \hbar} = \delta^i_j I$, a linear transformation will not be able to change the number of generators, and therefore the spaces are no longer isomorphic. The algebra of operators, then, must be part of the definition of the state space of a physical system. Additionally, this inclusion fixes the topology.

In Reverse Physics, there is a consistent general result that topologies in physical theories correspond to experimental verifiability: the open sets correspond to experimentally verifiable statements. [cite us, kevin kelly, ...] Therefore, if a system is fully characterized by a set of defining observables and their probability distribution, it makes sense that their topology (i.e. how we identify states experimentally) is fully characterized by the observables and the Born rule (i.e. the inner product). The above definition, then, is both justified on physical grounds and works in practice.
\begin{condition}[Quantum states]{QST:V}
	The state space of the system is represented by the projective space $P(V)$ of a quantum vector space $V$.
\end{condition}
...
It should be clear that this does not solve all problems. For example, it clearly lack some maximality condition on the Lie algebra so that it covers, in some sense, the whole space. Also, $\<\psi \mid (X + I) \psi \>$ is technically summing expectations of position expressed in meters with a probability, which is a dimensionally incongruent. So, representing observables as linear operators is, very likely, a shortcut. But, again, the idea is neither to solve all the problems at once, which leads to paralysis, not to ignore it, which leads to lack of progress. We just need to make local incremental progress in the direction of a global total solution.

\textbf{Section summary.} To sum up, the topology of the state space is connected to experimental verifiability, as open sets correspond to what is experimentally accessible. Hilbert spaces do not have the right topology. To fix this, the vector space must come additionally equipped with a set of defining observables and their commutation relationship, which is what we already do anyway, and the topology is the one generated by those observables and the inner product, because those are exactly the entities that tell us how we distinguish states.
...
\begin{condition}[Quantum states are rays]{QP-RAY}
	The state space of the system is represented by the projective space $P(V)$ of a complex topological vector space $V$.
\end{condition}
...
Finite-dimensional complex projective spaces are a symplectic manifold, therefore they allow classical Hamiltonian mechanics and classical statistical mechanics.  Therefore, when studying the classical Larmor precession, we can claim that the ``state of the magnetic dipole is a ray in a complex Hilbert space'' because the projective space for a two-dimensional quantum system is exactly a sphere, the Bloch sphere. In the infinite dimensional case, the projective space cease to be a manifold, which is a requirement for classical mechanics. It is an open question whether \ref{QP-RQY} would be compatible with a classical field theory in that case. Overall, the space of pure states is, in general, not enough even to distinguish between classical and quantum states.
...
\begin{condition}[Mutual exclusivity conditions]{[B]MUTEX}
	Let $\psi_i \in P(\mathcal{H})$ be a countable sequence of states. Then the following are equivalent:
	\begin{itemize}
...
\begin{condition}[Conditional expectation from Born rule]{BR-BORN}
	The conditional probability $p : P(V) \times P(V) \to [0,1]$ is given by the Born rule. That is, $p(\psi|\phi) = \frac{\< \phi | \psi \>\< \psi | \phi \>}{\< \psi | \psi \>\< \phi | \phi \>}$.
\end{condition}
\begin{condition}[Von Neumann entropy]{BR-ENT}
	The entropy function $S : D(V) \to \mathbb{R}$ is given by the Shannon entropy: $S(\rho) = - \tr(\rho \log \rho)$.
\end{condition}
\begin{condition}[Orthogonality is mutual exclusivity]{BR-ORME}
	Orthogonal states are mutually exclusive.
\end{condition}
\begin{Theorem}
	Over \ref{QE-DENS}+\ref{[B]MUTEX}, conditions \ref{BR-BORN}, \ref{BR-ENT} and \ref{BR-ORME} are equivalent. Moreover, \ref{QE-DENS}+\ref{[B]MUTEX} does imply \ref{BR-BORN}.
\end{Theorem}

Condition \ref{[B]MUTEX} captures the idea that the same concept, the mutual exclusivity of preparations $A$ and $B$, can be characterized in two equivalent ways. In terms of conditional probability, if we prepared $A$, there is zero probability of finding something that $B$ could have prepared; in terms of information entropy, we can use $A$ and $B$ to transfer a whole bit of entropy. Since these are necessary property of mutual exclusivity, \ref{[B]MUTEX} is a base condition. The equivalence between \ref{BR-BORN} and \ref{BR-ENT} stems from the fact that the conditional probability between two states is an invertible function of the entropy of their equal mixture. Note that \ref{BR-BORN} implies that orthogonal states are mutually exclusive, and therefore \ref{BR-ORME}. Since every density operator can be orthogonally decomposed, \ref{BR-ORME} implies \ref{BR-ENT} through \ref{[B]MUTEX}.

The Born rule, then, has the implicit assumption that mutually exclusive preparations exist, which is not required by \ref{QE-DENS}. A counter-example would be an ensemble space, either classical or quantum, in which there is an entropy cutoff and pure states are not reachable, even as a limit. While each state can still be reconstructed statistically through a tomography, it cannot be confirmed with a single-shot measurement, effectively because all preparations are too noisy. No pair of preparations would allow to reliably transfer one bit. We do not see (yet) any fundamental reason as to why this case should be ruled out a priori, and in may as well be that states in a future more fundamental theory have this property.

\textbf{Probability requires mutual exclusivity.} The crucial insight is that the convex structure of the ensemble space is not enough to define probability, it requires this addition \textbf{entropy structure}. The mixture coefficients $p_i$ of a mixture $\sum_i p_i \psi_i$ can be interpreted as probability if and only if $\psi_i$ are mutually exclusive. Kolmogorov probability, in fact, is based on the assumption that all the elements of the sample space are mutually exclusive. In this light, contextuality is more than justified: there is no alternative.

The independence of the entropic structure from the convex structure of the ensemble space is present in classical mechanics as well: the Liouville measure is not unique for a classical space, and exactly correspond to the entropy of uniform distributions in each region. Therefore it is not possible to recostruct the entropy from the convex set of probability measures alone. Since entropy is a property of ensembles, this is yet another hint that both classical and quantum mechanics are, at their core, statistical theories, theories about ensembles.
...
1) Equivalence between lots of conditions

\section{Projections as equilibration processes}

1) Not all measurements are projections
2) Continuous operators cannot have projections
3) Projections are processes with equilibria

Analogy with thermodynamics

\section{Classical mechanics as high-entropy limit}

\section{States as equilibria}
