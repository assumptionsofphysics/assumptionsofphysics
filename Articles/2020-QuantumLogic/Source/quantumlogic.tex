...
This situation, however, is formally identical to the previous one: before we had that $q_o \vee r_o \neq q_i \vee r_i$, now we have $p_f \vee q_f \neq p_o \vee q_o$. It would be improper to say that an electron has always either $x$-spin up or $y$-spin up just because it is so after the third step. Similarly, it would be improper as well to claim that the electron has always either $y$-spin up or down just because it is so after the measurement of spin along the $y$ axis. Neither the disjunction, nor its atomic components tell us anything about an inherent property associated with the observable $S_y$ possessed by a quantum particle independently of any measurement of $S_y$.
